...
if [[ -z "$REACT_APP_TAB_NAME" ]]; then
    echo -e "${YELLOW}⚠️  REACT_APP_TAB_NAME não definida, usando valor padrão: MultiChat${NC}"
    TAB_NAME="MultiChat"
else
...
# Substitui todas as ocorrências de "MultiChat" pelo novo nome
sed -i "s/MultiChat/$TAB_NAME/g" public/index.html
...
if [[ -f "public/manifest.json" ]]; then
    sed -i "s/MultiChat/$TAB_NAME/g" public/manifest.json
